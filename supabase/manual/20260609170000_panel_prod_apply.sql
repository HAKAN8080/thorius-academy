-- Prod kontrol + eksik grant düzeltmesi (idempotent)
-- Supabase SQL Editor'de tek seferde çalıştırın.

-- 1) Panel profil alanları + eğitmen başvuruları (zaten varsa atlar)
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS phone text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS bio text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS avatar_url text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS wp_user_id integer;
...
-- 2) profiles GRANT (profil sayfası + admin upsert için gerekli)
...
-- 3) API şema önbelleği
NOTIFY pgrst, 'reload schema';

-- 4) Doğrulama (tek sonuç tablosu — SQL Editor son sorguyu gösterir)
SELECT
  (SELECT COUNT(*) FROM public.enrollments) AS enrollment_count,
  (SELECT COUNT(*) FROM public.instructor_applications) AS application_count,
