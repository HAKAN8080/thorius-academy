...
import { KOCLUK_WELLNESS_CATEGORY_CONTENT } from "@/lib/course/kocluk-wellness-category-content";
...
async function main(): Promise<void> {
  ...
  for (const content of KOCLUK_WELLNESS_CATEGORY_CONTENT) {
    ...
    if (curriculum && curriculumColumnsReady) {
      ...
      for (const section of sections ?? []) {
        ...
        if (!dryRun) {
          await admin.from("sections").update({ title_en: titleEn }).eq("id", section.id);
        }
        sectionUpdates += 1;
      }
      ...
      for (const lesson of lessons ?? []) {
        ...
        if (!dryRun) {
          await admin.from("lessons").update({ title_en: titleEn }).eq("id", lesson.id);
        }
        lessonUpdates += 1;
      }
    }
