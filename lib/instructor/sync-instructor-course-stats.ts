...
export async function backfillAllInstructorCourseStatsFromCache(): Promise<{
  instructors: number;
  rowsUpserted: number;
}> {
  const admin = getSupabaseAdmin();
  ...
  for (const instructor of instructors ?? []) {
    rowsUpserted += await ensureInstructorCourseStatsFromCache(
      instructor.wp_user_id as number,
    );
  }
